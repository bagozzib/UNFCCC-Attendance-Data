...
# to remove the simple warnings
import warnings
warnings.filterwarnings('ignore')

# all the packages necessary for classifying the data.

import functools
import pandas as pd

# packages to translate in other languages to English, for better understanding and Classification
import argostranslate.package
import argostranslate.translate
import argostranslate

# "Spacy" and "Flair" Packages for "Named Entity Recognition" of Person Names and Affiliation Names.
import spacy
...
# Load the Large English NLP model, used for "Lemitizing" the words, check for "Stop Words" and for "Named Entity Recognition".
nlp = spacy.load("en_core_web_lg")
matcher = Matcher(nlp.vocab)

the_keyword_mis_split_list = []

may_be_wrong_split_items = []
failed_error_special_chars = []
...
def join_based_on_stopwords(data):
    ...
    def check_merge_conditions(current_str_last_word, next_str_first_word):
        """ This function returns "True", if the input 2 text items in the list can be merged else "False" """
...
class SegregateData(object):
    """ Classify the data to Group, Entity, Title, Name, Name doubt, English Translated Job Title, Job Title,
        ...
        we have added Name Doubt, English Translated columns, Extras, to make the final manual corrections easier. """
    ...
    def check_multi_words_cndtn(self, text, multiple_words_items_list):
        """ returns "True" if multiple words are present in the "same Column" or "List item" """
    ...
    def split_text_at_separator(self, input_data):
        """ Splits each item in the input_data at separator words and returns a list of split items. """
            ...
            for idx, part in enumerate(parts):
                ...
                if part in self.separator_items_set:
                    ...
                    if preceding_word not in self.separator_items_set and preceding_word not in stop_words:
                        ...
                        current_output = ""
                    # Add the separator word to the current output
                    current_output += part + " "
                else:
    ...
    def empty_division_store_affiliation(self, division, affiliation):
        """ If the Division is empty, store Affiliation in Division columns, if the text matches condition """
    ...
    def empty_job_division_join_affiliation_name(self, name, doubt, affiliation):
        """ Joins affiliation text with name if it's a person name; otherwise, adds to doubt. """

        # Remove leading and trailing whitespaces from affiliation
        affiliation = affiliation.strip()

        # Count the number of words in the affiliation
        affiliation_words_len = len(affiliation.split(' '))

        # Check if the affiliation is a single word and not a valid affiliation
        if affiliation_words_len == 1 and self.check_affiliation(affiliation) != 'true':
            # Check if the affiliation is a person name
            if self.check_person(affiliation):
                # If affiliation is a person name, add it to the name
                name += ' ' + affiliation
            else:
                # If affiliation is not a person name, add it to doubt
                doubt += affiliation

            # Set affiliation to empty string
            affiliation = ''

        # Return updated name, doubt, and affiliation
        return name, doubt, affiliation

    def set_strict_division_words(self, job_title, division, affiliation):
        """
        Adjusts job_title, division, and affiliation based on certain conditions.

        If both division and job_title are provided and job_title contains 'Presidency of',
        splits job_title at 'Presidency of' and assigns the first part to job_title and the
        rest to division.

        If division is not provided and affiliation is provided, and the affiliation is found
        in strict_division_keywords, assigns affiliation to division and sets affiliation to an empty string.
        """
        if division and job_title:
            # Splitting job_title at 'Presidency of' if it exists
        ...
        if not division and affiliation:
            # Checking if the affiliation is in strict_division_keywords
    ...
    def is_name_split_abruptly(self, name):
        """ Checks if a name appears to be split abruptly based on certain conditions. """
        try:
            # Split the name by whitespaces and consider the last word
            last_word = name.split()[-1]

            # Check if the last character or the last word itself is in names_end_join_set
            if last_word[-1].lower() in names_end_join_set or last_word.lower() in names_end_join_set or len(name.split()) == 1:
                return True

        except IndexError:
            # Handle the case where the name cannot be split due to IndexError
            pass
    ...
    def join_split_names(self, name, doubt, job_title, division, affiliation):
        """ Joins split names back with other fields if the name appears to be split abruptly. """
        # Handle name split with some characters at the end.
        name = name.strip()
        if self.is_name_split_abruptly(name):
            rem_text = ''
            # Determine which field to join with the name based on priority
            if doubt.strip():
                rem_text = doubt
                doubt = ''
            ...
            # Join the rem_text with the name
            name += ' ' + rem_text
    ...
    def is_doubt_col_split_abruptly(self, doubt):
        """ Checks if the doubt column appears to be split abruptly based on certain conditions. """

        # Split the doubt column into words and consider the first word
        first_word = doubt.split()[0].lower()

        # Check if the first word is in names_end_join_set or if there's an intersection with doubt_cols_abrupt_split_set
        if first_word in names_end_join_set or set(doubt.split()).intersection(doubt_cols_abrupt_split_set):
            return True
    ...
    def join_split_doubt_col(self, name, doubt, job_title):
        """ Joins split doubt column with name or job_title under certain conditions."""
        # If doubt is not empty and appears to be split abruptly, join it with name
        if doubt.strip() and self.is_doubt_col_split_abruptly(doubt):
            name = 'color:blue_name ' + name + ' ' + doubt
            doubt = ''

        # If doubt is not empty, job_title exists, and the first word of job_title is in doubt_merge_job_title_set,
        # join doubt with job_title and clear doubt
    ...
    def join_us_keyword(self, data_list):
        """ Joins occurrences of "U.S." with the subsequent word if it's not already a separate string."""
        for item in range(len(data_list)-1):
            # Check if "U.S." appears in the current item and if the next item is not empty
            if len(data_list[item].lower().split('u.s.')) > 1 and data_list[item + 1].split():
                # Join "U.S." with the subsequent word
                data_list[item + 1] = 'U.S.' + ' ' + data_list[item + 1]
                # Remove "U.S." from the current item
                data_list[item] = ' '.join(data_list[item].split()[:-1])

        return data_list

    def single_name_doubt_words_join_name(self, name, doubt):
        """
        Joins a single-word doubt with the name, adding a color tag to the name.
        """
        # Get the number of words in the doubt
        doubt_len = len(list(doubt.split()))

        # If the doubt consists of a single word, join it with the name and clear doubt
        if doubt_len == 1:
            name = 'color:blue_name ' + name + ' ' + doubt
            doubt = ''
    ...
    def job_title_affiliation_shift(self, name, job_title, division, affiliation):
        """ Shifts parts of the job title to division or affiliation under certain conditions. """
        # Make a copy of the original job title
        job_title_dup = job_title

        # Flag to track if job title was processed
        job_title_processed = False

        # Process each word in the job title
        for word in job_title.split():
            lower_word = word.lower()
            if lower_word in job_title_allowed_titles_set:
                # Find the index of the title in the job title
                index = job_title.lower().find(lower_word)
                # Extract the part of job title after the title
                post_title = job_title[index + len(lower_word):].strip()

                # Check if the post title part does not start with any negation word/phrase
                if post_title and not any(post_title.startswith(negation) for negation in job_title_negation_set):
                    # Trim job title up to the end of the title
                    job_title = job_title[:index + len(lower_word)].strip()
                    job_title_ext_text = post_title

                    # Shift parts of job title to division or affiliation based on conditions

                    # If division is absent and affiliation is present
                    if not division and affiliation:
                        division = job_title_ext_text

                    # If division is present and affiliation is present
                    elif division and affiliation:
                        division = job_title_ext_text + ' ' + job_title

                    # If division is absent and affiliation is also absent
                    elif not division and not affiliation:
                        affiliation = job_title_ext_text

                    # If division is present and affiliation is absent
                    elif division and not affiliation:
                        division, affiliation = job_title_ext_text, division

        # Check if job title was modified
        if job_title_dup != job_title:
            job_title_ext_split_list_names.append(name)

        return name, job_title, division, affiliation
    ...
    def split_text_by_keyword(self, text, classify_text):
        """
        Splits text into sentences based on a specified keyword.
        """
        sentences = []
        words = text.split()
        ...
        i = 0
        while i < len(words):
            if self.translate_text(words[i]).lower() == classify_text_words_lst[0] and i < len(words) - 1 and self.translate_text(words[i + 1]).lower() == classify_text_words_lst[1] and i >= 1 and (self.translate_text(words[i - 1]) not in ministry_of_stop_words):
    ...
    def separate_and_classify_text_by_keyword(self, name, doubt, job_title, division, affiliation, extras, en_trans_job_title, en_trans_division, en_trans_affiliation, en_trans_extras):
        """
        Separates and classifies text fields by keywords.
        """
        # Handle doubt field
        if doubt:
            regex = r'(de|del|van)$'
        ...
        try:
            # Handle Ministry of Government classification condition
        ...
        if division.split():
            ...
            if division_last_word in minister_of_startswith_ignore_list:
                # if the division is 'Federal' and the affiliation is 'Ministry of Economic'.
    ...
    def classify_data_final_stage(self, job_title, division, affiliation, extras):
        """
        Classifies data in the final stage based on specific conditions.
        """
    ...
    def handle_division_field(self, job_title, division, affiliation):
        """
        Handles the division field by determining its placement with job title or affiliation.
        """
        if affiliation:
            if self.contains_division_name(affiliation):
    ...
    def join_split_data_across_cols(self, name, input_list, special_chars_set):
        """
        Joins split data across columns based on special characters.
        """
        common_chr_cnt = 0
        start_idx = None
        for idx, string in enumerate(input_list):
            try:
                ...
                if common_chr_cnt == 2:
                    ...
                    list_items = input_list[idx].split(common_char_fnd)

                    if len(list_items) > 2:
                        may_be_wrong_split_items.append(list_items)

                    list_items = [item for item in list_items if item]
                    before_special_chr, after_special_chr = list_items
                ...
                failed_error_special_chars.append(name)
    ...
    def contains_enclosed_words(self, input_list):
        """
        Combines words enclosed within special characters with the preceding word.
        """
    ...
    def set_strict_affiliation_words(self, data_cols_list):
        """
        Sets strict affiliation words based on predefined patterns.
        """
    ...
    def job_title_handle_the_keyword_correctly(self, job_title, division, affiliation):
        """
        Handles the job title when it contains specific keywords.
        """
        split_txt = None
    ...
    def division_handle_the_keyword_correctly(self, division, affiliation):
        """
        Handles the division when it contains specific keywords.
        """
        split_txt = None
    ...
    def handle_full_stop_at_end(self, data_list):
        """
        Handles cases where a full stop is present at the end of strings.
        """
    ...
    def handle_job_tite_text_keywords(self, job_title, division, affiliation):
        """
        Handles specific keywords in the job title field.
        """
    ...
    def correct_job_keywords_for_job_title(self, name, job_title, en_trans_job_title, division):
        """
        Corrects specific keywords in the job title field.
        """
        # check for '-'
    ...
    def classify_extracted_data(self, processed_data):
        ...
        for item in processed_data:
            ...
            # similarly if there is a single word in extras also we will merge it with Affiliation, and the end ofr manual verification, we are coloring the text with blue color.
            ...
            doubt, job_title, division, affiliation = self.join_split_data_across_cols(name, data_cols_list, special_chars_set)
            ...
            name, doubt = self.single_name_doubt_words_join_name(name, doubt)
            name, job_title, division, affiliation = self.job_title_affiliation_shift(name, job_title, division, affiliation)
            ...
            if not division and (job_title and affiliation):
                job_title, affiliation = join_based_on_stopwords([job_title, affiliation])
        ...
        print('\\' * 100)
        print('may_be_wrong_split_items', may_be_wrong_split_items)
        print('\\' * 100)
        print('failed_error_special_chars', failed_error_special_chars)
        print('\\' * 100)
